...
async function main() {
  ...
  console.log('┌─────────┬──────────────┬──────────────┬──────────────────────┬──────────┐');
  console.log('│ Venda   │ Valor Total  │ Devolvido    │ Cliente              │ Crédito  │');
  console.log('├─────────┼──────────────┼──────────────┼──────────────────────┼──────────┤');
  ...
  problemas.forEach((p) => {
    ...
    console.log(`│ ${vendaId} │ ${total} │ ${devolvido} │ ${cliente} │ ${credito} │`);
  });
  ...
  if (problemasRestantes.length === 0) {
    console.log('✅ Validação OK! Todas as devoluções parciais foram corrigidas.\n');
  } else {
    console.log(`⚠️  Ainda existem ${problemasRestantes.length} problema(s) pendente(s).\n`);
  }
}
