 ...
 */
export async function backfillDataPagamento(dryRun: boolean = false): Promise<void> {
  console.log('='.repeat(60));
  console.log('🔄 BACKFILL: data_pagamento em vendas antigas');
  console.log('='.repeat(60));
  console.log(`Modo: ${dryRun ? '🔍 DRY RUN (simulação)' : '✍️ EXECUÇÃO REAL'}`);
  console.log('');
  ...
  try {
    ...
    const { data: vendasSemData, error: fetchError } = await supabase
      .from('vendas')
      .select('id, status_pagamento, data_pagamento, data_venda, created_at, updated_at')
    ...
    if (!vendasSemData || vendasSemData.length === 0) {
      ...
      console.log('   Todas as vendas pagas já têm data de pagamento registrada.');
      return;
    }

    console.log(`📦 Encontradas ${vendasSemData.length} vendas pagas sem data_pagamento`);
    console.log('');

    // 2. Processar cada venda e determinar a melhor data
    const updates: Array<{ id: number; data_pagamento: string; fonte: string }> = [];
    ...
    const estatisticas = updates.reduce((acc, update) => {
    ...
    updates.slice(0, 5).forEach((update) => {
      ...
      console.log(`     └─ data_pagamento será: ${new Date(update.data_pagamento).toLocaleString('pt-BR')}`);
      console.log(`     └─ origem: ${update.fonte}`);
      console.log(`     └─ data_venda original: ${new Date(venda!.data_venda).toLocaleString('pt-BR')}`);
    });
    ...
    for (let i = 0; i < updates.length; i += batchSize) {
      ...
      const promises = batch.map(async (update) => {
        ...
        if (error) {
          console.error(`   ❌ Erro ao atualizar venda #${update.id}:`, error.message);
          return false;
        }
        return true;
      });
...
if (typeof window !== 'undefined') {
  ...
  console.log('   1. Para simular (dry run): await backfillDataPagamento(true)');
